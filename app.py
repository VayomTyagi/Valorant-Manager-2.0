...
class MainApp:
    ...
    def create_widgets(self):
        ...
        self.hide_unhide_checkbox = ctk.CTkSwitch(self.buttons_frame, text="Streamer mode",variable=self.streamermode_var, onvalue="on", offvalue="off")
    ...
    def create_agent_button(self, parent, agent_name, original_image_path, row, col):
        ...
        image_button = ctk.CTkButton(master=parent, image=photo, text=agent_name, height=150, width=102,font=("Helvetica", 14), hover_color="#000080", compound='top')
    ...
    def on_agent_button_click(self, agent_name, original_image_path):
        ...
        while not self.agent_lock_event.is_set() and not self.stop_agent_lock_process:
                ...
                if max_val >= 0.8:
                    pg.click(x=max_loc[0] + 41, y=max_loc[1] + pg.size()[1] // 2 + 30, button='left')
                    time.sleep(0.01)
                    pg.click(x=max_loc2[0] + 30, y=max_loc2[1] + pg.size()[1] // 2 + 30, button='left')
                    self.update_output(f"{agent_name} locked successful")
                    break
